...
begin
  ...
  // все объекты TaskThreadList будут уничтожены в TAPI.StopAllTask
  TaskThreadList.Free;
...
begin
  ...
  TaskSearchFiles := TTaskSearchFiles.Create(TaskID, NameDLL, NameTask,
    ProgressBar);
...
begin
  ...
  TaskSearchSequence := TTaskSearchSequence.Create(TaskID, NameDLL, NameTask,
    ProgressBar);
...
begin
  ...
  TaskProcessCli := TTaskProcessCli.Create(TaskID, NameDLL, NameTask,
    ProgressBar);
...
begin
  ...
  if (TaskThread <> nil) and (not TaskThread.Finished) then
  begin
    TaskThread.StopTask;
...
begin
  ...
  // методы Remove, Clear, Free автоматически уничтожают объекты TaskList,
  // если OwnsObjects = True (значение по умолчанию)
