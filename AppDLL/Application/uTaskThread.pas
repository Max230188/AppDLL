...
type
  ...
  protected
    ...
    // методы для работы с событиями объявляются как динамические, а не как виртуальные
    procedure DoTaskTerminate; dynamic;
...
begin
  Task := nil; // объект будет уничтожен в TAPI.DeleteTaskFromList
...
begin
  TaskSearchFiles := nil; // объект будет уничтожен в TAPI.DeleteTaskFromList
...
begin
  StartSearchFiles;
  // метод который синхронизируется с помощью Synchronize не может иметь параметров
...
begin
  TaskSearchSequence := nil; // объект будет уничтожен в TAPI.DeleteTaskFromList
...
begin
  TaskProcessCli := nil; // объект будет уничтожен в TAPI.DeleteTaskFromList
